...
const HomeScreen: React.FC<Props> = props => {
  ...
  const handleSave = async (transaction: ITransaction, actions) => {
    actions.setSubmitting(true)

    // setError(null)
    setIsLoading(true)
    try {
      console.log(`SAVING | HomeScreen: ${Object.entries(transaction)}`)
      await POST_TRANSACTION(transaction)
    } catch (e) {
      // setError(e.message)
      setIsLoading(false)
    }
          ...
          >
            {({
              ...
              <Layout level='3' style={styles.modalContainer}>
                ...
                <Datepicker icon={CalendarIcon} date={date} onSelect={setDate(values.date)} />
